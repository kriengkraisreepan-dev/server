...
A bill/receipt's `items[]` are merged by product identity (`productId` + unit price) across every POS order that fed into the bill before they are stored — a product ordered in more than one round (more Leos later, not just the first) is one line with the summed quantity/total, not one line per order. A genuine price change mid-tab (e.g. Happy Hour ending) still keeps its own separate line, since the merge key includes price. See `CombinedBillingService#itemSnapshot`/`#mergeItems`.
...
`seatTables[]` is a new, additive top-level array for customers who sit down and order food/drink without playing a billiard table — a bar or lounge zone, not a physical snooker table. Each entry is `{ id, code, name, status, nickname }` (`status`: `free`, `occupied`, or `awaiting_payment` — mirrors a table's own status vocabulary) with no `relay`, `pricingProfileId`, or `items[]` — seats carry no hardware and no timed session. `name` is the zone's permanent name, managed as a plain named list from Settings (add/rename/remove), not counted like `tables[]`. `nickname` is a separate, transient label for whoever is currently occupying the seat (e.g. "คุณเอ") — settable by anyone who can start a POS order (not Settings-gated) — that survives bill creation and is only cleared once payment is actually confirmed.

`posOrders[]` gains a third `orderType`, `SEAT`, alongside `WALK_IN`/`TABLE`, with `seatId`/`seatName` snapshot fields (parallel to `tableId`/`tableName`) and `tableSessionId` always `null`. A seat's first confirmed order flips it to `occupied`.

Bills gain `saleSource: "SEAT"` alongside `TABLE`/`WALK_IN`/`LEGACY`, plus a `seatId` field (parallel to `tableSessionId`) so a payment-confirm or bill-reopen can find its way back to the right seat — `tableId` is deliberately left `null` for a seat bill (same as walk-in) since seats and tables are separate, independently-numbered collections and must never be confused via a shared id. A seat bill is modeled on a walk-in bill (`playAmount`/`tableCharge` always 0) but aggregates every `CONFIRMED`+`UNBILLED` `SEAT` order for that seat into one bill rather than exactly one order — closer to a table's tab than to a single walk-in sale. Existing `playAmount`/`foodAmount` report aggregation needs no changes: a zero-play-amount bill already reports as pure POS revenue. Creating the bill moves the seat to `awaiting_payment` (not straight to `free`, exactly like a table) so a checkout that never gets confirmed still visibly owes money; the payment-confirm route is what actually frees the seat and clears its nickname.
