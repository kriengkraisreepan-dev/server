...
async function sellDrinksOnPartialBill(base, headers) {
  // Editing a product needs the product-screen password step-up (see requireElevation in index.js).
  await fetch(`${base}/api/auth/elevate`, { method: "POST", headers, body: JSON.stringify({ scope: "products", password: "123456789" }) });
...
test("RETURN_TO_TAB un-bills the drinks without touching stock, and the final checkout charges them", async t => {
  ...
  const response = await fetch(`${base}/api/bills/${sale.bill.id}`, { method: "DELETE", headers, body: JSON.stringify({ password: "123456789", reason: "คิดเงินผิดวิธี", voidMode: "RETURN_TO_TAB" }) });
...
test("CANCEL_RESTORE_STOCK puts the drinks back on the shelf and charges nothing", async t => {
  ...
  const response = await fetch(`${base}/api/bills/${sale.bill.id}`, { method: "DELETE", headers, body: JSON.stringify({ password: "123456789", reason: "กดผิด ลูกค้ายังไม่ได้รับของ", voidMode: "CANCEL_RESTORE_STOCK" }) });
  assert.equal(response.status, 200);
...
test("CANCEL_KEEP_STOCK writes the drinks off — consumed, not charged, not restocked", async t => {
  ...
  const response = await fetch(`${base}/api/bills/${sale.bill.id}`, { method: "DELETE", headers, body: JSON.stringify({ password: "123456789", reason: "แถมให้ลูกค้า", voidMode: "CANCEL_KEEP_STOCK" }) });
  assert.equal(response.status, 200);
...
test("RETURN_TO_TAB is refused once the table has closed and reopened for the next customer", async t => {
  ...
  const response = await fetch(`${base}/api/bills/${sale.bill.id}`, { method: "DELETE", headers, body: JSON.stringify({ password: "123456789", reason: "ลองย้ายกลับ", voidMode: "RETURN_TO_TAB" }) });
...
test("an unknown void mode is rejected and changes nothing", async t => {
  ...
  const sale = await sellDrinksOnPartialBill(base, headers);
  const response = await fetch(`${base}/api/bills/${sale.bill.id}`, { method: "DELETE", headers, body: JSON.stringify({ password: "123456789", reason: "x", voidMode: "WHATEVER" }) });
