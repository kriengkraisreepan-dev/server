...
class BackendSupervisor {
  // host defaults to loopback-only (127.0.0.1). Pass host: "" (empty string) to request LAN
  // mode instead — index.js treats an empty/unset LUCKY_HOST as "bind all interfaces"
  // (`process.env.LUCKY_HOST || undefined`), so "" must be forwarded as-is, never defaulted.
  constructor({ entry, programRoot, dataRoot, port, host = "127.0.0.1", logFile, spawn = fork, now = () => Date.now(), maxRestarts = 3, restartWindowMs = 300000, readinessTimeoutMs = 30000 } = {}) { Object.assign(this, { entry, programRoot, dataRoot, port, host, logFile, spawn, now, maxRestarts, restartWindowMs, readinessTimeoutMs }); this.child = null; this.ownedPid = null; this.restarts = []; this.stopping = false; this.ready = false; }
  environment() { return { ...process.env, ELECTRON_RUN_AS_NODE: "1", LUCKY_DESKTOP_MODE: "1", LUCKY_INTERNAL_TEST: process.env.LUCKY_INTERNAL_TEST || (process.env.LUCKY_ELECTRON_TEST_DATA_ROOT ? "1" : "0"), LUCKY_DATA_ROOT: this.dataRoot, LUCKY_PROGRAM_ROOT: this.programRoot, LUCKY_WORKSPACE_ROOT: this.programRoot, LUCKY_ALLOW_WORKSPACE_TEST_ROOT: process.env.LUCKY_ALLOW_WORKSPACE_TEST_ROOT || "0", LUCKY_HOST: this.host, PORT: String(this.port) }; }
