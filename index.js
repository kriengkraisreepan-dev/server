...
// Screens that need the password re-entered even inside an open session, because the shop computer
// is shared and staff can sit down at whatever account was left logged in. Minutes = how long the
...
app.post("/api/products", requirePermission(PERMISSIONS.PRODUCT_MANAGE), requireElevation("products"), (req, res) => { try { res.status(201).json(inventoryService.createProduct(req.body || {}, actorId(req))); } catch (error) { res.status(error.message.includes("already exists") ? 409 : 400).json({ error: error.message }); } });
app.patch("/api/products/:id", requirePermission(PERMISSIONS.PRODUCT_MANAGE), requireElevation("products"), (req, res) => { try { res.json(inventoryService.updateProduct(req.params.id, req.body || {}, actorId(req))); } catch (error) { res.status(error.message === "Product not found" || error.message === "Category not found" ? 404 : error.message.includes("already exists") ? 409 : 400).json({ error: error.message }); } });
app.patch("/api/products/:id/status", requirePermission(PERMISSIONS.PRODUCT_MANAGE), requireElevation("products"), (req, res) => { try { res.json(inventoryService.changeProductStatus(req.params.id, req.body?.status, actorId(req))); } catch (error) { res.status(error.message === "Product not found" ? 404 : 400).json({ error: error.message }); } });
app.post("/api/products/:id/stock/receive", requirePermission(PERMISSIONS.INVENTORY_MANAGE), requireElevation("products"), (req, res) => { try { res.json(inventoryService.receiveStock(req.params.id, req.body || {}, actorId(req))); } catch (error) { res.status(error.message === "Product not found" ? 404 : 400).json({ error: error.message }); } });
app.post("/api/products/:id/stock/adjust", requirePermission(PERMISSIONS.INVENTORY_MANAGE), requireElevation("products"), (req, res) => { try { res.json(inventoryService.adjustStock(req.params.id, req.body || {}, actorId(req))); } catch (error) { res.status(error.message === "Product not found" ? 404 : 400).json({ error: error.message }); } });
...
app.post("/api/product-categories", requirePermission(PERMISSIONS.PRODUCT_MANAGE), requireElevation("products"), (req, res) => { try { res.status(201).json(inventoryService.createCategory(req.body || {}, actorId(req))); } catch (error) { res.status(error.message.includes("already exists") ? 409 : 400).json({ error: error.message }); } });
app.patch("/api/product-categories/:id", requirePermission(PERMISSIONS.PRODUCT_MANAGE), requireElevation("products"), (req, res) => { try { res.json(inventoryService.updateCategory(req.params.id, req.body || {}, actorId(req))); } catch (error) { res.status(error.message === "Category not found" ? 404 : error.message.includes("already exists") ? 409 : 400).json({ error: error.message }); } });
app.patch("/api/product-categories/:id/status", requirePermission(PERMISSIONS.PRODUCT_MANAGE), requireElevation("products"), (req, res) => { try { res.json(inventoryService.changeCategoryStatus(req.params.id, req.body?.status, actorId(req))); } catch (error) { res.status(error.message === "Category not found" ? 404 : 400).json({ error: error.message }); } });
...
// Shared by every "undo a stuck awaiting_payment bill" path: the automatic ones right after a
// failed create-bill call (no payment ever attached) and the manual one a cashier can trigger later
// on a bill that WAS given a payment record, but the customer never actually paid it (they walked
// away, or staff started a checkout and closed the dialog). The manual path must cancel that
// leftover pending payment first — the automatic paths never had one to begin with.
function reopenAwaitingBill(bill, actor, reason) {
  if (!bill || bill.status !== "awaiting_payment") { const error = new Error("Bill is not awaiting payment"); error.code = "BILL_NOT_AWAITING_PAYMENT"; throw error; }
  billingRepository.payments().filter(payment => payment.billId === bill.id && payment.status === "pending").forEach(payment => paymentService.cancelPayment(payment.id));
  reopenCouponForBill(bill, actor);
  combinedBillingService.reopenUnpaidBill(bill, actor, reason);
}
...
app.post("/api/pos-orders/:id/create-bill", requirePermission(PERMISSIONS.TABLE_CLOSE), (req, res) => { let createdBill=null; try { const preview=combinedBillingService.previewWalkInBilling(req.params.id), reward=normalizeRewardRequest({...req.body,memberId:req.body?.memberId??preview.memberId}); if(shouldRedeem(reward,"WALK_IN")) memberService.previewRedeem(rewardPreviewInput({memberId:reward.memberId,totalSatang:preview.totalSatang,saleSource:"WALK_IN"}),reward.redeemedPoints,settingsService.getSettings());
  ...
  const result = combinedBillingService.createWalkInBill(req.params.id, actorId(req)); createdBill=result.bill; const coupon=applyCouponToBill(result.bill, actorId(req)); const { payment, payments } = createBillPayments(result.bill, req.body?.paymentMethod, req.body?.splitPayments, actorId(req)); res.json({ ...result, payment, payments, coupon }); } catch (error) { if(createdBill&&createdBill.status==="awaiting_payment")reopenAwaitingBill(createdBill,actorId(req)); if(String(error.code||"").startsWith("COUPON_"))return couponError(res,error); if(error.code&&error.code!=="ORDER_ALREADY_BILLED"&&error.code!=="ORDER_NOT_FOUND")return sendRewardError(res,error); const status = error.code === "ORDER_ALREADY_BILLED" ? 409 : error.code === "ORDER_NOT_FOUND" ? 404 : 400; res.status(status).json({ error: error.code || "WALK_IN_BILL_ERROR", message: error.message, details: error.details }); } });
...
// Cancelling a seat's last remaining confirmed-unbilled order is the "no bill exists yet" cancel
// path (see cancelSeatOrdersDialog in app.js) — the seat has never been through the
// awaiting_payment/paid dance, so nothing frees it automatically the way payment confirmation does.
app.post("/api/pos-orders/:id/cancel", requirePermission(PERMISSIONS.POS_ORDER_CANCEL_DRAFT), async (req, res) => { try { const order = await posOrderService.cancelOrder(req.params.id, req.body || {}, req.user); if (order.orderType === "SEAT" && order.status === "CANCELLED") { const seat = seatById(order.seatId); const stillOpen = (store.posOrders || []).some(item => item.orderType === "SEAT" && String(item.seatId) === String(order.seatId) && item.status === "CONFIRMED" && item.billingStatus === "UNBILLED"); if (seat && !stillOpen && seat.status === "occupied") { seat.status = "free"; seat.nickname = null; save(); } } res.json({ order }); } catch (error) { posOrderError(res, error); } });
...
// Cancelling from the table card no longer makes the tab disappear — it becomes a "pending_review"
// bill (time used + products, who cancelled, why) for the owner to check and close out with Void.
// A table already awaiting payment has a real bill in flight; that one is paid or backed out of with
// the card's own ชำระเงิน / ยกเลิก buttons, not by cancelling the session underneath it.
app.post("/api/tables/:id/cancel", async (req, res) => { try { const table = tableById(req.params.id), session = sessionRepository.findOpenSessionByTable(req.params.id); if (!table || !session) return res.status(409).json({ error: "ไม่มี Session ที่ยกเลิกได้" }); if (session.state === "AWAITING_PAYMENT") return res.status(409).json({ error: "TABLE_AWAITING_PAYMENT", message: "โต๊ะนี้มีบิลรอชำระอยู่ — ใช้ปุ่มชำระเงินหรือยกเลิกบิลค้างชำระแทน" }); const reviewBill = combinedBillingService.createTableReviewBill(session.id, { reason: req.body?.reason, actorId: actorId(req) }); sessionService.cancelSession(session.id); const reservedCoupon = couponService.reservedForSession(session.id); if (reservedCoupon) couponService.release(reservedCoupon.id, "SESSION_CANCELLED", actorId(req)); billingService.audit("SESSION_CANCELLED", { tableId: table.id, sessionId: session.id, billId: reviewBill.id, actorId: actorId(req) }); const relay = await setRelayState(table, "off"); save(); res.json({ table: enrichTable(table), reviewBill, warning: relay.failed ? "ยกเลิก Session แล้ว แต่ติดต่อ ESP32 เพื่อปิด Relay ไม่สำเร็จ" : undefined }); } catch (error) { res.status(409).json({ error: error.message }); } });
...
function createCombinedCheckout(sessionId, paymentMethod, requestActorId, rewardInput={}) {
  ...
  try {
    ...
    if(createdBill&&createdBill.status==="awaiting_payment")reopenAwaitingBill(createdBill,requestActorId);
    throw error;
  }
}
...
// A nickname is who's currently sitting there (e.g. "คุณเอ"), set by whoever can start a tab —
// same permission as creating the seat's first order, not the Settings-gated permanent rename above.
app.patch("/api/seats/:id/nickname", requirePermission(PERMISSIONS.POS_ORDER_CREATE), (req, res) => { try { store.seatTables = seatTableService.setNickname(store.seatTables || [], req.params.id, req.body?.nickname); save(); res.json({ seats: store.seatTables.map(enrichSeat) }); } catch (error) { res.status(seatErrorStatus(error.code)).json({ error: error.code || "SEAT_ERROR", message: error.message }); } });
...
// "ยกเลิกบิล" on an occupied seat: the whole open tab becomes one "pending_review" bill and the seat
// is freed — same rule as cancelling a table from its card, so the same roles that can open a seat's
// tab can cancel it, and the owner reviews it afterwards (see createSeatReviewBill).
app.post("/api/seats/:id/cancel", requirePermission(PERMISSIONS.POS_ORDER_CREATE), (req, res) => { try { const seat = seatById(req.params.id); if (!seat) return res.status(404).json({ error: "SEAT_NOT_FOUND", message: "ไม่พบโซนที่นั่ง" }); if (seat.status !== "occupied") return res.status(409).json({ error: "SEAT_NOT_OCCUPIED", message: seat.status === "awaiting_payment" ? "โซนนี้มีบิลรอชำระอยู่ — ใช้ปุ่มชำระเงินหรือยกเลิกบิลค้างชำระแทน" : "โซนนี้ไม่มีรายการให้ยกเลิก" }); const result = combinedBillingService.createSeatReviewBill(seat.id, { reason: req.body?.reason, actorId: actorId(req) }); res.json({ reviewBill: result.bill, seat: enrichSeat(result.seat) }); } catch (error) { res.status(seatErrorStatus(error.code)).json({ error: error.code || "SEAT_CANCEL_ERROR", message: error.message }); } });
...
app.post("/api/seats/:id/create-bill", requirePermission(PERMISSIONS.TABLE_CLOSE), (req, res) => { let createdBill=null; try { const result = combinedBillingService.createSeatBill(req.params.id, actorId(req)); createdBill=result.bill; const { payment, payments } = createBillPayments(result.bill, req.body?.paymentMethod, req.body?.splitPayments, actorId(req)); res.json({ ...result, payment, payments }); } catch (error) { if(createdBill&&createdBill.status==="awaiting_payment")reopenAwaitingBill(createdBill,actorId(req)); res.status(seatErrorStatus(error.code)).json({ error: error.code || "SEAT_BILL_ERROR", message: error.message }); } });
// Standalone "give up on this checkout" for a bill that already has its own real pending payment
// record (unlike the automatic reopens above, which fire only when no payment ever got created) —
// e.g. a cashier opened a checkout, closed the dialog without confirming or cancelling, and the
// table/seat/order has been stuck "awaiting_payment" since. Same permission as creating the bill in
// the first place, so whoever can check someone out can also back out of a stuck one.
app.post("/api/bills/:id/reopen", requirePermission(PERMISSIONS.TABLE_CLOSE), (req, res) => { try { const bill = billingRepository.findBill(req.params.id); if (!bill) return res.status(404).json({ error: "BILL_NOT_FOUND", message: "ไม่พบบิล" }); reopenAwaitingBill(bill, actorId(req), req.body?.reason || "ยกเลิกโดยพนักงาน — ลูกค้าไม่ได้ชำระเงิน"); res.json({ reopened: true }); } catch (error) { res.status(error.code === "BILL_NOT_AWAITING_PAYMENT" ? 409 : 400).json({ error: error.code || "BILL_REOPEN_ERROR", message: error.message }); } });
...
app.post("/api/payments/:id/confirm", requirePermission(PERMISSIONS.PAYMENT_CONFIRM), async (req, res) => { let redeemedBill=null; try { const pending=billingRepository.findPayment(req.params.id), pendingBill=pending&&billingRepository.findBill(pending.billId); if(pendingBill?.redeemSelected&&pendingBill.saleSource==="TABLE"&&!pendingBill.redeemApplied){memberService.redeemPoints(pendingBill,actorId(req));redeemedBill=pendingBill;} const { bill, payment } = paymentService.confirmPayment(req.params.id, actorId(req)); if(bill.status!=="paid"||bill.partialOrdersOnly){save();return res.json({bill,payment});} memberService.earn(bill,actorId(req),settingsService.getSettings()); const table = tableById(bill.tableId); const session = table && sessionRepository.findOpenSessionByTable(table.id); if (session) sessionService.completeSession(session.id); billingService.audit("SESSION_CLOSED_AFTER_PAYMENT", { tableId: bill.tableId, sessionId: session?.id, billId: bill.id, paymentId: payment.id, actorId: actorId(req) }); const relay = table ? await setRelayState(table, "off") : {}; if (table) sessionRepository.releaseTable(table.id);
  // A seat table has no session to release it — the seat itself carries "awaiting_payment" while a
  // bill is outstanding (mirrors table.status), and only returns to free (nickname cleared, ready
  // for the next customer) once payment is actually confirmed here.
  if (bill.seatId) { const seat = seatById(bill.seatId); if (seat) { seat.status = "free"; seat.nickname = null; } }
  save(); res.json({ bill, payment, warning: relay.failed ? "ยืนยันชำระแล้ว แต่ติดต่อ ESP32 เพื่อปิด Relay ไม่สำเร็จ" : undefined }); } catch (error) { if(redeemedBill&&redeemedBill.status!=="paid") memberService.rollbackRedeem(redeemedBill,actorId(req)); res.status(409).json({ error: error.message }); } });
...
app.delete("/api/bills/:id", requirePermission(PERMISSIONS.BILL_VOID), (req, res) => { try { const bill = billingRepository.findBill(req.params.id); if (!bill) return res.status(404).json({ error: "ไม่พบบิลที่ต้องการยกเลิก" });
  ...
  // Only unwind the table if it is still on THIS bill's session. It used to cancel whatever session
  // was open on the table: voiding yesterday's bill — or a pending-review bill from a tab cancelled
  // hours ago — while the next customer was playing killed their game and freed the table under them.
  if (!bill.partialOrdersOnly) { const table = tableById(bill.tableId); const session = table && sessionRepository.findOpenSessionByTable(table.id); const ownSession = session && (!bill.tableSessionId || session.id === bill.tableSessionId); if (ownSession) sessionService.cancelSession(session.id); if (table && (!session || ownSession)) sessionRepository.releaseTable(table.id); }
  // A seat waiting on this bill has nothing left to wait for once it is voided — its orders are now
  // cancelled or written off, not owed — so it goes back to free like a table would.
  if (bill.seatId) { const seat = seatById(bill.seatId); const stillWaiting = (store.bills || []).some(other => other.id !== bill.id && String(other.seatId) === String(bill.seatId) && other.status === "awaiting_payment"); if (seat && seat.status === "awaiting_payment" && !stillWaiting) { seat.status = "free"; seat.nickname = null; } }
