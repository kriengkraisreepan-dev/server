...
app.get("/api/state", requireAuth, (req, res) => res.json({ settings: settingsService.getSettings(), tables: store.tables.map(enrichTable), members: store.members, products: inventoryService.listProducts({ pageSize: 1000 }, req.user.role).items, posOrders: store.posOrders || [], bills: store.bills, payments: store.payments, auditLogs: store.auditLogs || [], user: req.user }));
...
app.post("/api/pos-orders/:id/create-bill", requirePermission(PERMISSIONS.TABLE_CLOSE), (req, res) => { try { const result = combinedBillingService.createWalkInBill(req.params.id, actorId(req)); const { payment } = paymentService.createPayment({ billId: result.bill.id, method: req.body?.paymentMethod || "cash", amountSatang: result.bill.totalSatang, actorId: actorId(req) }); res.json({ ...result, payment }); } catch (error) { const status = error.code === "ORDER_ALREADY_BILLED" ? 409 : error.code === "ORDER_NOT_FOUND" ? 404 : 400; res.status(status).json({ error: error.code || "WALK_IN_BILL_ERROR", message: error.message, details: error.details }); } });
...
function createCombinedCheckout(sessionId, paymentMethod, requestActorId) { const result = combinedBillingService.createBill(sessionId, requestActorId); const { payment } = paymentService.createPayment({ billId: result.bill.id, method: paymentMethod || "cash", amountSatang: result.bill.totalSatang, actorId: requestActorId }); return { ...result, payment }; }
app.get("/api/table-sessions/:id/billing-preview", requirePermission(PERMISSIONS.TABLE_CLOSE), (req, res) => { try { res.json({ preview: combinedBillingService.buildPreview(req.params.id) }); } catch (error) { res.status(error.code === "DUPLICATE_BILL" ? 409 : 400).json({ error: error.code || "BILLING_PREVIEW_ERROR", message: error.message }); } });
app.post("/api/table-sessions/:id/create-bill", requirePermission(PERMISSIONS.TABLE_CLOSE), (req, res) => { try { res.json(createCombinedCheckout(req.params.id, req.body?.paymentMethod, actorId(req))); } catch (error) { res.status(error.code === "DUPLICATE_BILL" ? 409 : 400).json({ error: error.code || "COMBINED_BILL_ERROR", message: error.message }); } });
app.post("/api/tables/:id/checkout", requirePermission(PERMISSIONS.TABLE_CLOSE), (req, res) => { try { const session = sessionRepository.findOpenSessionByTable(req.params.id); if (!session || !["ACTIVE", "PAUSED"].includes(session.state)) return res.status(400).json({ error: "โต๊ะไม่ได้อยู่ในสถานะที่คิดเงินได้" }); res.json(createCombinedCheckout(session.id, req.body?.paymentMethod, actorId(req))); } catch (error) { res.status(error.code === "DUPLICATE_BILL" ? 409 : 400).json({ error: error.code || "COMBINED_BILL_ERROR", message: error.message }); } });
app.post("/api/payments/:id/confirm", requirePermission(PERMISSIONS.PAYMENT_CONFIRM), async (req, res) => { try { const { bill, payment } = paymentService.confirmPayment(req.params.id, actorId(req)); memberService.earn(bill,actorId(req)); const table = tableById(bill.tableId); const session = table && sessionRepository.findOpenSessionByTable(table.id); if (session) sessionService.completeSession(session.id); billingService.audit("SESSION_CLOSED_AFTER_PAYMENT", { tableId: bill.tableId, sessionId: session?.id, billId: bill.id, paymentId: payment.id, actorId: actorId(req) }); const relay = table ? await setRelayState(table, "off") : {}; if (table) sessionRepository.releaseTable(table.id); save(); res.json({ bill, payment, warning: relay.failed ? "ยืนยันชำระแล้ว แต่ติดต่อ ESP32 เพื่อปิด Relay ไม่สำเร็จ" : undefined }); } catch (error) { res.status(409).json({ error: error.message }); } });
