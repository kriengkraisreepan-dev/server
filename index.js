...
const posOrderService = new PosOrderService(posOrderRepository, inventoryService, { audit: (event, actorId, data) => billingService.audit(event, { actorId, data }), findTable: tableById });
posOrderService.normalizeLegacyPosOrders();
const combinedBillingService = new CombinedBillingService({ sessionRepository, sessionService, posOrderRepository, billingRepository, billingService, inventoryService, getMemberName: memberId => memberById(memberId)?.name || "ลูกค้าทั่วไป", save });
