class IntegrityCheckService {
  constructor({ store, reservations = () => [], deposits = () => [] }) {
    this.store = store;
  ...
  run() {
    ...
    const billIds = new Set((store.bills || []).map(item => item.id));
    ...
    for (const deposit of deposits) {
      if (deposit.status === "SETTLED" && (!deposit.settledBillId || !billIds.has(deposit.settledBillId))) add("ERROR", "SETTLED_DEPOSIT_BILL_MISSING", "DEPOSIT", deposit.id, deposit.settledBillId || "missing settledBillId");
    ...
    duplicate(store.bills || [], "receiptNumber", "BILL", "DUPLICATE_RECEIPT_NUMBER");
    duplicate(store.bills || [], "number", "BILL", "DUPLICATE_BILL_NUMBER");
    ...
    for(const event of store.auditLogs||[]){const key=event.details?.idempotencyKey;if(key&&idempotencyKeys.has(key))add("ERROR","DUPLICATE_REVENUE_IDEMPOTENCY_KEY","AUDIT",event.id,key);if(key)idempotencyKeys.add(key);}
