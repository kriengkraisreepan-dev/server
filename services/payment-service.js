...
class PaymentService {
  ...
  createPayment({ billId, method, amountSatang, actorId = "SYSTEM", allowPartial = false }) {
    ...
    // Only reject when something is ALREADY reserved and covers the bill — a bill whose amount due
    // is legitimately zero (e.g. fully discounted, see discount-billing-route.test.js) must still
    // accept its first (zero-amount) payment; reserved===0 there, so this must not fire for it.
    if (reserved > 0 && remaining <= 0) throw new Error("Bill already has a payment");
    if (allowPartial) {
