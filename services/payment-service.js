...
class PaymentService {
  ...
  createPayment({ billId, method, amountSatang }) {
    ...
    requireNonNegativeSatang(amountSatang, "payment amount"); if (amountSatang < bill.totalSatang) throw new Error("Payment amount is less than bill total");
    ...
    this.repository.savePayment(payment); this.billingService.audit("PAYMENT_CREATED", { tableId: bill.tableId, billId, paymentId: payment.id, data: { method, amountSatang } }); return { bill, payment };
  }
  confirmPayment(paymentId) { const payment = this.repository.findPayment(paymentId); if (!payment || payment.status !== "pending") throw new Error("Payment is not pending"); const bill = this.repository.findBill(payment.billId); if (!bill) throw new Error("Bill not found"); if (!["awaiting_payment", "pending"].includes(bill.status)) throw new Error("Bill is not awaiting payment"); if (payment.amountSatang < bill.totalSatang) throw new Error("Payment amount is less than bill total"); payment.status = "paid"; payment.paidAt = this.now(); this.repository.savePayment(payment); this.billingService.markPaid(bill); this.billingService.audit("PAYMENT_CONFIRMED", { tableId: bill.tableId, billId: bill.id, paymentId: payment.id, data: { method: payment.method || "qr", amountSatang: payment.amountSatang } }); return { bill, payment }; }
