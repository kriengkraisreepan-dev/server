...
const DEFAULTS = Object.freeze({ timeZone: "Asia/Bangkok", currency: "THB", dateTimeFormat: "th-TH", roundingRule: "UP_TO_BAHT", backupIntervalHours: 24, esp32: { enabled: false, baseUrl: "" }, security: { timeoutMinutes:480, warningMinutes:5, maxLoginAttempts:5, lockDurationMinutes:15 }, rewards: { rewardPointValue: 1, rewardMinimumPoints: 100, allowPartialRedeem: true, allowRedeemTable: true, allowRedeemWalkIn: false }, loyalty: { tablePointsPerHour: 5, tablePointIntervalMinutes: 60, tablePointRounding: "FLOOR" }, defaultPricingProfileId: "default" });
function normalizeSettings(current) {
  ...
  return { ...DEFAULTS, ...current, roundingRule: "UP_TO_BAHT", hourlyRateSatang, minimumChargeSatang, hourlyRate: Number(satangToBaht(hourlyRateSatang)), minimumCharge: Number(satangToBaht(minimumChargeSatang)), pricingProfiles: pricingProfiles.map(profile => ({ ...profile, roundingRule: "UP_TO_BAHT" })), esp32: { ...DEFAULTS.esp32, ...(current.esp32 || {}) }, security: { ...DEFAULTS.security, ...(current.security || {}) }, rewards: { ...DEFAULTS.rewards, ...(current.rewards || {}), allowRedeemWalkIn:false }, loyalty: { ...DEFAULTS.loyalty, ...(current.loyalty || {}), tablePointRounding:"FLOOR" } };
}
class SettingsService {
  constructor(repository) { this.repository = repository; }
  getSettings() { return normalizeSettings(this.repository.get()); }
  updateSettings(changes) {
    const current = this.getSettings(), next = { ...current, ...changes, esp32: { ...current.esp32, ...(changes.esp32 || {}) }, rewards: { ...current.rewards, ...(changes.rewards || {}), allowRedeemWalkIn:false }, loyalty: { ...current.loyalty, ...(changes.loyalty || {}), tablePointRounding:"FLOOR" } };
