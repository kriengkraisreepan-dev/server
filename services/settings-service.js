...
const DEFAULTS = Object.freeze({ timeZone: "Asia/Bangkok", currency: "THB", dateTimeFormat: "th-TH", roundingRule: "UP_TO_BAHT", backupIntervalHours: 24, backupExternalPath: "", esp32: { enabled: false, baseUrl: "" }, hardware: { setupWizardEnabled: true, discoveryEnabled: true, wifiProvisioningEnabled: true, setupApEnabled: true, setupButtonEnabled: false, automaticWifiRecoveryEnabled: false }, security: { timeoutMinutes:480, warningMinutes:5, maxLoginAttempts:5, lockDurationMinutes:15 }, rewards: { rewardPointValue: 1, rewardMinimumPoints: 100, allowPartialRedeem: true, allowRedeemTable: true, allowRedeemWalkIn: false }, loyalty: { tablePointsPerHour: 5, tablePointIntervalMinutes: 60, tablePointRounding: "FLOOR" }, reservation: { defaultDepositAmount: 100, minimumDepositAmount: 100, depositRequired: true, autoAssignTable: true, autoLightOn: true, allowLightBeforeCheckIn: true, checkInGraceMinutes: 60, autoApplyDeposit: true, allowManualDepositRemoval: false, alertEnabled: true, alertSoundEnabled: true, alertSoundVolume: 80, alertRepeatMinutes: 1, deferMinutes: 20, deferredPriorityEnabled: true, autoForfeitNoShowDeposit: true }, defaultPricingProfileId: "default" });
...
class SettingsService {
  ...
  updateSettings(changes) {
    ...
    ["checkInGraceMinutes", "alertRepeatMinutes", "deferMinutes"].forEach(key => { if (!Number.isInteger(Number(next.reservation[key])) || Number(next.reservation[key]) < 1) throw new Error(`${key} must be a positive integer`); });
