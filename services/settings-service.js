...
const DEFAULTS = Object.freeze({ timeZone: "Asia/Bangkok", currency: "THB", dateTimeFormat: "th-TH", roundingRule: "NONE", backupIntervalHours: 24, esp32: { enabled: false, baseUrl: "" }, defaultPricingProfileId: "default" });
function normalizeSettings(current) {
  ...
  return { ...DEFAULTS, ...current, hourlyRateSatang, minimumChargeSatang, hourlyRate: Number(satangToBaht(hourlyRateSatang)), minimumCharge: Number(satangToBaht(minimumChargeSatang)), pricingProfiles: pricingProfiles.map(profile => ({ ...profile })), esp32: { ...DEFAULTS.esp32, ...(current.esp32 || {}) } };
}
class SettingsService {
  ...
  updateSettings(changes) {
    ...
    next.pricingProfiles = (next.pricingProfiles || []).map(normalizePricingProfile); if (!next.pricingProfiles.some(profile => profile.id === next.defaultPricingProfileId)) throw new Error("defaultPricingProfileId must exist in pricingProfiles");
