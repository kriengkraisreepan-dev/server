...
class TableSessionService {
  ...
  //
  // Counted against BILLABLE seconds rather than the wall clock, so pausing the table pauses the
  // countdown too — otherwise a table paused for twenty minutes would announce that the customer's
  // time was up while nobody was playing on it.
  normalizePlannedSeconds(value) { const seconds = Math.round(Number(value) || 0); if (!Number.isInteger(seconds) || seconds < 0) { const error = new Error("Planned time must be a non-negative number of seconds"); error.code = "INVALID_PLANNED_TIME"; throw error; } if (seconds > 24 * 3600) { const error = new Error("Planned time cannot exceed 24 hours"); error.code = "INVALID_PLANNED_TIME"; throw error; } return seconds; }
  setPlannedSeconds(sessionId, plannedSeconds) { const session = this.requireSession(sessionId); if (![STATES.ACTIVE, STATES.PAUSED].includes(session.state)) { const error = new Error("Only a running or paused table can have its planned time changed"); error.code = "SESSION_NOT_RUNNING"; throw error; } session.plannedSeconds = this.normalizePlannedSeconds(plannedSeconds); return this.repository.saveSession(session); }
  // Negative means they are over the time they were given — the caller decides what to do about it.
  remainingSeconds(session) { const planned = Number(session?.plannedSeconds || 0); return planned ? planned - this.billableSeconds(session) : null; }
