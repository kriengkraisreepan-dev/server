...
class MemberService{
 ...
 assertUnique(input,id){const code=clean(input.memberCode),phone=clean(input.phone),email=clean(input.email).toLowerCase();if(!code)throw new Error("Member code is required");if(this.repository.members().some(m=>m.id!==id&&(m.memberCode||m.code)===code))throw new Error("Member code already exists");if(phone&&this.repository.members().some(m=>m.id!==id&&clean(m.phone)===phone))throw new Error("Phone already exists");if(email&&this.repository.members().some(m=>m.id!==id&&clean(m.email).toLowerCase()===email))throw new Error("Email already exists");}
 ...
 list(q={}){this.normalize();const text=clean(q.search||q.q).toLowerCase(),status=clean(q.status);return this.repository.members().filter(m=>(!status||m.status===status)&&(!text||[m.memberCode,m.displayName,m.phone,m.email].some(v=>clean(v).toLowerCase().includes(text))));}
