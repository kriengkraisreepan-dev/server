...
class MemberService{
 ...
 // `segments` are the bill's rateSegments — the stretches the session was actually charged at. Pass
 // them and each completed hour is awarded at its own rate's points (ซ้อมเดี่ยว / Happy Hour / ปกติ);
 // omit them and every hour earns the shop-wide rate, which is what walk-in and pre-segment bills do.
 calculateTablePoints(playSeconds,settings={},{segments=null}={}){const loyalty=settings.loyalty||settings,intervalMinutes=Math.max(1,Number(loyalty.tablePointIntervalMinutes||60)),defaultPointsPerInterval=Math.max(0,Number(loyalty.tablePointsPerHour??5));
  const earned=calculateTablePoints({playSeconds,segments,intervalMinutes,defaultPointsPerInterval});
  return {...earned,policy:{mode:"TABLE_TIME",tablePointsPerHour:defaultPointsPerInterval,intervalMinutes,rounding:"FLOOR",perRate:earned.breakdown}};}
 earn(bill,actor,settings={}){if(!bill.memberId||bill.pointsEarnedApplied||bill.saleSource!=="TABLE")return null;const m=this.repository.findById(bill.memberId);if(!m||m.status!=="ACTIVE")return null;const earned=this.calculateTablePoints(bill.playDurationSeconds,settings,{segments:bill.rateSegments||null}),before=Number(m.points||0);
